...
public class TestController implements ResourceLoaderAware {
    ...
    public void servletContextGetResourceAsStream(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
		    resourceStream = request.getSession().getServletContext().getResourceAsStream(path);
    ...
    @RequestMapping(value = "/classLoaderGetResource")
    public void classLoaderGetResource(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String path = getRequiredStringParameter(request, "path");
        String output = "<html><head><title>Testing path [" + path + "]</title></head><body>";
        InputStream resourceStream = null;
		try {
		    resourceStream = getClass().getClassLoader().getResource(path).openStream();
            output += "From ClassLoader via path [" + path + "]: first char: [" + (char) resourceStream.read() + "]";
		    resourceStream.close();
            output += "</body></html>";
            response.getWriter().write(output);
		} finally {
