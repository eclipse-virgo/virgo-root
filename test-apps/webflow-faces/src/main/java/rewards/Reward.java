...
public class Reward {
	...
	/**
	 * Reduces the amount of the reward by the given amount. The reward amount
	 * however will not go below 0.0.
	 * @param reduceByAmount the amount to reduce by
	 */
	public void reduce(MonetaryAmount reduceByAmount) {
		amount = amount.subtract(reduceByAmount);
		if (amount.getDouble() < 0.0) {
			amount = MonetaryAmount.valueOf("0.0");
		}
	}
}