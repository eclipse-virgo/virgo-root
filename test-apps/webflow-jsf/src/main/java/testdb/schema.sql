...
create table T_ACCOUNT (
	...
	NUMBER varchar(9), NAME varchar(50) not null, 
	CREDIT_CARD varchar(16), 
	unique(NUMBER));

create table T_ACCOUNT_PROFILE (
	ACCOUNT_ID integer, NAME varchar(50), 
	DATE_OF_BIRTH date not null, 
    EMAIL varchar(80) not null, 
    REWARDS_NEWSLETTER char(1) not null, 
    MONTHLY_EMAIL_UPDATE char(1) not null, 
    unique(ACCOUNT_ID));
...
alter table T_ACCOUNT_PROFILE add constraint FK_ACCOUNT_ACCOUNT_DETAIL 
	foreign key (ACCOUNT_ID) references T_ACCOUNT(ID) on delete cascade;
